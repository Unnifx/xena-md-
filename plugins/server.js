...
System({
    ...
    await message.reply(`_Xena-md is shutting down..._`);
...
System({
  ...
  if (!match) return await message.reply(`Example: .setvar SUDO:919074403240`);
  const [key, ...part] = match.split(":");
  const value = part.join(":").trim();
  if (!key || !value) return await message.send(`_*Example: .setvar SUDO:919074403240*_`);
  if (server === "HEROKU") {
